import { ShoppingBag, MousePointer2, ArrowLeft } from "lucide-react";
...
const CURSOR_OPTIONS: { id: CursorType; name: string; icon: ReactNode; desc: string }[] = [
  ...
  { 
    ...
    desc: 'Dibuja un rastro fluido de tinta al moverte.' 
  },
  { 
    id: 'crosshair', 
    name: 'Mira Cinética', 
  ...
  { 
    ...
    icon: (
      <div className="relative w-12 h-12 flex items-center justify-center">
        ...
        <svg width="24" height="24" viewBox="0 0 24 24" fill="var(--color-high-yellow)" className="drop-shadow-[0_0_8px_rgba(255,223,0,1)]">
          <path d="M12 2L14.5 9.5L22 12L14.5 14.5L12 22L9.5 14.5L2 12L9.5 9.5L12 2Z" />
        </svg>
      </div>
    ), 
    desc: 'Genera lluvia de partículas doradas. Clic para estallar.' 
  },
  { 
    id: 'neon-glow', 
    name: 'Luz de Neón', 
    icon: (
      <div className="relative w-16 h-16 flex items-center justify-center overflow-hidden">
        <div className="absolute w-12 h-12 rounded-full blur-[8px] bg-gradient-to-r from-pink-500 to-transparent mix-blend-multiply opacity-60 translate-x-[-8px]" />
    ...
    <div className="w-full min-h-screen p-8 bg-[var(--color-paper)] relative flex flex-col items-center">
      ...
      <div className="w-full flex items-center justify-between z-10 mb-8 mt-4">
        ...
        <h1 className="text-4xl font-headline font-black flex items-center gap-3">
          <ShoppingBag size={40} /> Tienda
        </h1>
        <div className="w-[100px]"></div> {/* Spacer */}
      </div>

      {/* Tabs a ancho completo */}
      <div className="w-full flex gap-4 border-b-4 border-[var(--color-ink)] pb-4 mb-8">
        <button className="flex items-center gap-2 px-6 py-3 bg-[var(--color-high-yellow)] border-2 border-[var(--color-ink)] rounded-t-xl font-bold text-xl transform translate-y-4">
          <MousePointer2 size={24} /> Ratones
        </button>
        <button className="flex items-center gap-2 px-6 py-3 bg-white border-2 border-[var(--color-ink)] rounded-t-xl font-bold text-xl text-gray-400 cursor-not-allowed">
          Temas (Pronto)
        </button>
      </div>
